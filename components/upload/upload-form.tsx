...
export function UploadForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (!documentType) { toast.error('문서유형을 입력해주세요.'); return }
    ...
    <div>
        ...
        {formatBytes(file.size)}
          ...
          <div className="flex flex-col items-center gap-2">
            <div className={cn(
              ...
              <CloudUpload className={cn('w-7 h-7', dragging ? 'text-primary-foreground' : 'text-muted-foreground')} />
            </div>
            <div>
              <p className="font-semibold text-foreground text-sm">PDF 파일을 드래그하거나 클릭하여 선택</p>
              <p className="text-xs text-muted-foreground mt-0.5">PDF 형식만 지원 · 최대 50MB</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-border bg-muted/30">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">필수 항목</p>
        ...
        <div className="px-5 py-4 space-y-4">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">선택 항목</p>
