  ...
  return <div className="report-page">
    ...
    <div className="report-workspace">
      ...
      <div className="quality-report">
        ...
        <section><SectionTitle roman="I" title="VISUAL INSPECTION (외관검사)" plan="n=50 / Reel, Lot" /><div className="report-table visual-table"><div className="table-head"><span>NO.</span><span>INSPECTION ITEMS (검사항목)</span><span>A/c/Re</span><span>RESULT (판정)</span></div>{visualRows.map((row) => <div key={row[0]}><span>{row[0]}</span><span>{row[1]}</span><span>{row[2]}</span><span>{row[3]}</span></div>)}</div></section>
        ...
        <section><SectionTitle roman="III" title="PLATING THICKNESS (도금두께)" plan="n=50 / Reel, Lot" /><div className="report-table plating-table"><div className="table-head"><span>NO.</span><span>검사 항목</span><span>도금 종류</span><span>1차</span><span>2차</span><span>3차</span><span>판정</span></div>{Array.from({ length: 10 }, (_, i) => <div key={i}><span>{i + 1}</span><Input /><span></span><Input /><Input /><Input /><span>OK</span></div>)}</div></section>
        <section><SectionTitle roman="IV" title="FUNCTIONAL TEST (기능검사)" plan="n=100mm/EA, 2EA/Shipping, C=0" /><div className="report-table functional-table"><div className="table-head"><span>NO.</span><span>검사 항목</span><span>검사 기준</span><span>검사 결과</span><span>판정</span></div>{[['1', 'X-Ray 검사', 'N 1.0~2.0μm', '이상 없음'], ['2', '전기적 특성 검사', '정상 동작 확인', '이상 없음']].map((row) => <div key={row[0]}>{row.map((cell) => <span key={cell}>{cell}</span>)}<span>OK</span></div>)}</div></section>
