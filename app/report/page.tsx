  ...
  return <div className="report-page">
    ...
    <div className="report-workspace">
      ...
      <div className="quality-report">
        <header className="report-header-grid"><Field label="CUSTOMER(고객명)" value={values.customer} onChange={update('customer')} /><Field label="DESCRIPTION(품명)" value={values.product} onChange={update('product')} /><Field label="LOT NO.(로트번호)" value={values.lotNo} onChange={update('lotNo')} /><Field label="ISSUED DATE(발행일자)" value={values.issueDate} onChange={update('issueDate')} /><Field label="규격(재질)" value={values.specification} onChange={update('specification')} /><Field label="LOT SIZE(수량)" value={values.quantity} onChange={update('quantity')} /></header>
        ...
        <section><SectionTitle roman="II" title="DIMENSION INSPECTION (치수검사)" plan="n=100mm/Reel, Reel/Shipping, C=0" /><div className="report-table dimension-table"><div className="table-head"><span>NO.</span><span>검사 항목</span><span>규격</span><span>허용차</span><span>1차</span><span>2차</span><span>3차</span><span>판정</span></div>{dimensionRows.map((row) => <div key={row[0]}>{row.map((cell, i) => <span key={`${row[0]}-${i}`}>{cell}</span>)}</div>)}</div></section>
        <section><SectionTitle roman="III" title="PLATING THICKNESS (도금두께)" plan="X-Ray 검사 / Reel, Lot" /><div className="report-table plating-table"><div className="table-head"><span>NO.</span><span>검사 항목</span><span>도금 스펙</span><span>검사 결과</span><span>판정</span></div>{Array.from({ length: 10 }, (_, i) => <div key={i}><span>{i + 1}</span><Input value={i === 0 ? 'X-Ray 검사' : undefined} readOnly={i === 0} /><Input aria-label={`도금 스펙 ${i + 1}`} /><Input aria-label={`검사 결과 ${i + 1}`} /><span>OK</span></div>)}</div></section>
        <div className="report-bottom"><div className="sample-box"><div className="box-label">TEST SAMPLE</div><div className="sample-blank" aria-label="실제 시료 부착 영역" /></div><div className="approval-box"><div className="box-label">종합판정</div><div className="pass">PASS / OK</div><div className="approval-grid"><span>작성</span><span>승인</span><span>검토</span><span>고객</span><div /><div /><div /><div /></div></div></div>
      </div>
    </div>
  </div>
}
