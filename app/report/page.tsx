  ...
  return <div className="report-page">
    ...
    <div className="report-workspace">
      <aside className="report-editor no-print"><div className="report-editor-heading"><FileText /> <div><strong>성적서 정보</strong><p>입력한 값이 미리보기에 반영됩니다.</p></div></div><div className="report-editor-fields">
        ...
        {values.platingTypes.map((type, index) => <label key={index}>도금 {index + 1}<Input value={type} onChange={(e) => setValues((current) => ({ ...current, platingTypes: current.platingTypes.map((item, itemIndex) => itemIndex === index ? e.target.value : item) as [string, string, string] }))} /></label>)}
      </div></aside>
      <div className="quality-report">
        ...
        <section><SectionTitle roman="III" title="PLATING THICKNESS (도금두께)" plan="X-Ray 검사 / Reel, Lot" /><div className="report-table plating-table"><div className="table-head"><span>No.</span><span><em>μm</em><span className="plating-type-value">{values.platingTypes[0]}</span></span><span><em>μm</em><span className="plating-type-value">{values.platingTypes[1]}</span></span><span><em>μm</em><span className="plating-type-value">{values.platingTypes[2]}</span></span><span>판정</span></div>{Array.from({ length: 10 }, (_, i) => <div key={i}><span>{i + 1}</span><Input aria-label={`${i + 1}번 첫 번째 도금 두께`} /><Input aria-label={`${i + 1}번 두 번째 도금 두께`} /><Input aria-label={`${i + 1}번 세 번째 도금 두께`} /><span>OK</span></div>)}</div></section>
