  ...
  return <div className="report-page">
    ...
    <div className="report-workspace">
      ...
      <div className="quality-report">
        ...
        <section><SectionTitle roman="III" title="PLATING THICKNESS (도금두께)" plan="X-Ray 검사 / Reel, Lot" /><div className="report-table plating-table"><div className="table-head"><span>No.</span><span><em>μm</em><Input aria-label="첫 번째 도금 종류" placeholder="예: Sn" /></span><span><em>μm</em><Input aria-label="두 번째 도금 종류" placeholder="예: Ni" /></span><span><em>μm</em><Input aria-label="세 번째 도금 종류" placeholder="예: Au" /></span><span>판정</span></div>{Array.from({ length: 10 }, (_, i) => <div key={i}><span>{i + 1}</span><Input aria-label={`${i + 1}번 첫 번째 도금 두께`} /><Input aria-label={`${i + 1}번 두 번째 도금 두께`} /><Input aria-label={`${i + 1}번 세 번째 도금 두께`} /><span>OK</span></div>)}</div></section>
