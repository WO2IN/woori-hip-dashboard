...
export default function ReportWritingPage() {
  ...
  const [adhesionResult, setAdhesionResult] = useState('이상 없음')
  ...
  const reset = () => { setCustomer(''); setProduct(''); setLotNumber(''); setIssueDate(''); setSpecification(''); setLotSize(''); setInspectionDate(''); setAdhesionResult('이상 없음'); setVisualRows(initialVisualRows); setThicknessRows(initialThicknessRows); toast.success('입력 내용을 초기화했습니다.') }
  ...
  return (
    <main className="report-page min-h-full bg-muted/20 p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-6xl">
        ...
        <Tabs defaultValue="write" className="gap-5">
          ...
          <TabsContent value="write" className="flex flex-col gap-5">
            ...
            <Card><CardHeader><SectionTitle number="04" title="밀착성 시험 시료" description="인쇄한 성적서에 실제 시험 시료를 부착할 수 있는 공간입니다." /></CardHeader><CardContent className="grid gap-4 sm:grid-cols-[1fr_240px]"><div className="flex min-h-32 items-center justify-center rounded-lg border-2 border-dashed border-slate-400 bg-muted/20 text-center text-sm text-muted-foreground"><div><p className="font-medium text-foreground">밀착성 시험 시료 부착란</p><p className="mt-1 text-xs">시험 후 제품 시료를 이 영역에 부착하세요.</p></div></div><Field label="밀착성 시험 결과"><Input value={adhesionResult} onChange={e => setAdhesionResult(e.target.value)} placeholder="예: 이상 없음" /></Field></CardContent></Card>

            <Card className="report-preview"><CardHeader><SectionTitle number="04" title="성적서 미리보기" description="입력한 정보가 성적서에 어떻게 표시되는지 확인합니다." /></CardHeader><CardContent><div className="report-paper rounded-lg border-2 border-slate-400 bg-background p-5 text-sm shadow-sm"><div className="flex items-start justify-between border-b-2 border-primary pb-4"><div><img src="/logo.png" alt="WOORI 로고" className="mb-2 h-9 w-auto object-contain object-left" /><h3 className="text-xl font-bold tracking-tight">QUALITY ASSURANCE REPORT</h3><p className="text-xs text-muted-foreground">품질보증 성적서</p></div><div className="text-right text-xs"><p className="text-muted-foreground">성적서 No.</p><p className="font-semibold">{lotNumber || '미입력'}</p><p className="mt-1 text-muted-foreground">발행일자: {issueDate || '-'}</p></div></div><div className="mt-4 grid grid-cols-2 gap-px overflow-hidden rounded border bg-border sm:grid-cols-4"><div className="bg-muted/40 p-3"><p className="text-xs text-muted-foreground">고객명</p><p className="mt-1 font-medium">{customer || '-'}</p></div><div className="bg-muted/40 p-3"><p className="text-xs text-muted-foreground">품명</p><p className="mt-1 font-medium">{product || '-'}</p></div><div className="bg-muted/40 p-3"><p className="text-xs text-muted-foreground">규격</p><p className="mt-1 font-medium">{specification || '-'}</p></div><div className="bg-muted/40 p-3"><p className="text-xs text-muted-foreground">도금 종류</p><p className="mt-1 font-medium">{platingType}</p></div><div className="bg-muted/40 p-3"><p className="text-xs text-muted-foreground">LOT SIZE (수량)</p><p className="mt-1 font-medium">{lotSize || '-'}</p></div><div className="bg-muted/40 p-3"><p className="text-xs text-muted-foreground">검사일자</p><p className="mt-1 font-medium">{inspectionDate || '-'}</p></div></div><ReportPreviewSection title="I. 외관검사"><table className="w-full border-collapse text-xs"><thead><tr className="bg-muted/50"><th className="border px-2 py-2">NO.</th><th className="border px-2 py-2 text-left">검사 항목</th><th className="border px-2 py-2 text-left">규격 / 기준</th><th className="border px-2 py-2 text-left">검사값</th><th className="border px-2 py-2">판정</th></tr></thead><tbody>{visualRows.map(row => <tr key={`preview-visual-${row.id}`}><td className="border px-2 py-2 text-center">{row.id}</td><td className="border px-2 py-2">{row.item || '-'}</td><td className="border px-2 py-2">{row.standard || '-'}</td><td className="border px-2 py-2">{row.value || '-'}</td><td className="border px-2 py-2 text-center font-medium">{row.result || '-'}</td></tr>)}</tbody></table></ReportPreviewSection><ReportPreviewSection title="II. 도금두께"><table className="w-full border-collapse text-xs"><thead><tr className="bg-muted/50"><th className="border px-2 py-2">NO.</th><th className="border px-2 py-2 text-left">측정 포인트</th><th className="border px-2 py-2">도금 종류</th><th className="border px-2 py-2">도금두께 (μm)</th><th className="border px-2 py-2">판정</th></tr></thead><tbody>{thicknessRows.map(row => <tr key={`preview-thickness-${row.id}`}><td className="border px-2 py-2 text-center">{row.id}</td><td className="border px-2 py-2">{row.point || '-'}</td><td className="border px-2 py-2 text-center">{platingType}</td><td className="border px-2 py-2 text-center">{row.value || '-'}</td><td className="border px-2 py-2 text-center font-medium">{row.result || '-'}</td></tr>)}</tbody></table></ReportPreviewSection><div className="mt-5 flex items-center justify-between rounded border bg-primary/5 px-4 py-3"><span className="font-semibold">종합 판정</span><span className="text-xl font-bold text-primary">PASS / OK</span></div></div></CardContent></Card>
          </TabsContent>
      ...
      <style jsx global>{`
        ...
        .report-sample-slot { min-height: 8rem; }
        .report-paper::after { content: 'V. 밀착성 시험 시료\\A\\A시험 후 제품 시료를 이 영역에 부착하세요'; white-space: pre-wrap; display: flex; align-items: center; justify-content: center; min-height: 7rem; margin-top: 1.25rem; border: 1px dashed #8eaec8; color: #183b61; text-align: center; font-size: 0.72rem; }
        @media print {
